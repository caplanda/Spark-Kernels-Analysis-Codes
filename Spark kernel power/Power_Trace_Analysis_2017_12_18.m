...
%TO RUN 
%correct directory to your computer, the desired voltage data
%folder, the test sheet, and enter desired filename in csvwrite on line 106
...
%change directories on your computer
dataDir = '\\depot.engr.oregonstate.edu\users\castilet\Windows.Documents\Desktop\Spark kernel power\2017_12_18 Voltage Data';
testDir = '\\depot.engr.oregonstate.edu\users\castilet\Windows.Documents\Desktop\Spark kernel power';
TestSheetFile = '2017_12_18 test sheet.txt';
...
for count_1 = 1:length(dataName)
    ...
    Trace = csvread(file, 40, 0); %row offset to remove garbage leftover from previous putty recording
    ...
    for count_2 = 1:length(Trace)
        ...
        if (I_new<=0.0) && (I_old<=0.0) && (I_older<=0.0) %with new resistence values do not equal zero
    ...
    %% Mean of periods per trial linked with file name and pressure
...
fid = fopen('2017_12_18 E_mean Data.csv','w');
...
scatter(cell2mat(E_mean(:,2)),cell2mat(E_mean(:,3)),5)